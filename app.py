...
st.set_page_config(
    page_title="Расчет среднего балла за семестр (преобразование квалификатива в отметку до сотых без округления)",
    page_icon="📘",
    layout="centered"
)

st.title("📘 Расчет среднего балла за семестр (преобразование квалификатива в отметку до сотых без округления)")
st.write(
    "Загрузите Журнал по предмету, скачанный с платформы Studii.md"
    "Программа добавить в отчет еще одну колонку "Средний балл отметка", где будет выполнен расчет среднего арифмитического показателя за семестр путем преобразования каждого квалификатива в баллы"
