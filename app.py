...
st.set_page_config(
    page_title="Расчет среднего балла за семестр для I-IV классов (преобразование квалификатива в отметку с сотыми без округления)",
    page_icon="📘",
    layout="centered"
)

st.title("📘 Расчет среднего балла за семестр для I-IV классов (преобразование квалификатива в отметку с сотыми без округления)")
st.write(
    "Загрузите журнал по предмету, скачанный с платформы Studii.md. Переходите на страницу нужного предмета, где отображается журнал со всеми отметками учеников класса и выбираете Скачать журнал Excel.\n\n"
